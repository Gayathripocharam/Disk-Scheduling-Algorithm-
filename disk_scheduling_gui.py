...
class DiskSchedulingApp(ctk.CTk):
    ...
    def create_sidebar(self):
        ...
        # Direction
        self.dir_label = ctk.CTkLabel(self.sidebar_frame, text="SCAN/C-SCAN Dir:", anchor="w")
    ...
    def create_main_area(self):
        ...
        self.tab_cscan = self.tabview.add("C-SCAN Graph")
        ...
        self.lbl_fcfs = ctk.CTkLabel(self.card_fcfs, text="-", font=ctk.CTkFont(size=22))
        ...
        self.lbl_sstf = ctk.CTkLabel(self.card_sstf, text="-", font=ctk.CTkFont(size=22))
        ...
        self.lbl_scan = ctk.CTkLabel(self.card_scan, text="-", font=ctk.CTkFont(size=22))
        self.lbl_scan.pack(pady=(5, 20))

        # C-SCAN Card
        self.card_cscan = ctk.CTkFrame(self.results_frame, corner_radius=15)
        self.card_cscan.grid(row=0, column=3, padx=10, pady=10, sticky="nsew")
        self.lbl_title_cscan = ctk.CTkLabel(self.card_cscan, text="C-SCAN", font=ctk.CTkFont(size=18, weight="bold"))
        self.lbl_title_cscan.pack(pady=(20, 5))
        self.lbl_cscan = ctk.CTkLabel(self.card_cscan, text="-", font=ctk.CTkFont(size=22))
        self.lbl_cscan.pack(pady=(5, 20))

        self.lbl_best = ctk.CTkLabel(self.results_frame, text="Best Algorithm: -", font=ctk.CTkFont(size=20, weight="bold"), text_color="#00FF00")
        self.lbl_best.grid(row=1, column=0, columnspan=4, pady=(30, 10))
    ...
    def cscan(self, requests, head, direction):
        left = sorted([r for r in requests if r < head])
        right = sorted([r for r in requests if r >= head])
        disk_size = self.get_disk_size()
        ...
        if direction == "Right":
            ...
            seq += left[::-1] + [0] + [disk_size - 1] + right[::-1]
    ...
    def run_analysis(self):
        ...
        if cscan_mov == best_mov: 
            best_algos.append("C-SCAN")
            self.card_cscan.configure(border_width=2, border_color="#00FF00")
        ...
        self.plot_graph(cscan_seq, f"C-SCAN Disk Scheduling ({direction})", self.tab_cscan)
